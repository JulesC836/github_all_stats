...
const LANG_COLORS = {
    ...
    Vue: '#41b883', ASP: '#6a40fd', 'ASP.NET': '#9400ff'
};
...
async function generateSVG(data) {
    ...
    const profileHTML = `
        <g transform="translate(30, 30)">
            ...
            <text x="160" y="118" class="sec text-xs">🔗 ${escapeXML(user.blog || 'No website')}   👥 <tspan class="pri bold">${user.followers}</tspan> followers · <tspan class="pri bold">${user.following}</tspan> following   📅 Joined ${joinDate}</text>
        </g>
    `;
    ...
    const renderCard = (x, val, label, icon) => `
        <g transform="translate(${x}, 180)">
            ...
            <rect x="15" y="20" width="40" height="40" rx="8" fill="#11161d" />
    ...
    const streakSvg = `
        <g transform="translate(30, 280)">
            ...
            <g transform="translate(445, 80)">
                ...
                <!-- Simulating a progress ring -->
                <circle cx="0" cy="0" r="55" fill="none" stroke="url(#streakGradient)" stroke-width="6" stroke-dasharray="345" stroke-dashoffset="${345 - (345 * 0.9)}" stroke-linecap="round" transform="rotate(-90)" />
                
                <!-- Fire Icon -->
                <path fill="#f78166" transform="translate(-10, -25) scale(1.2)" d="M7.998 14.5c2.832 0 5-1.98 5-4.5 0-1.463-.68-2.19-1.879-3.383l-.036-.037c-1.013-1.008-2.3-2.29-2.834-4.434-.322.256-.63.579-.864.953-.432.696-.621 1.58-.046 2.73.473.947.67 2.284-.278 3.232-.61.61-1.545.84-2.403.525a2.167 2.167 0 01-1.216-1.084c-.706 1.153-.843 2.634-.137 3.974.597 1.135 1.694 2.024 4.693 2.024z"/>
    ...
    // 4. Languages and Detailed Stats
    // Processes languages
    ...
    sortedLangs.forEach(([lang, bytes], i) => {
        ...
        const width = (percent / 100) * 380;
        
        chartHTML += `<rect x="${currentX}" y="20" width="${width}" height="12" fill="${color}" rx="3"/>`;
        currentX += width;
        ...
        langListHTML += `
            <g transform="translate(${lx}, ${ly})">
                ...
                <text x="20" y="4" class="sec text-sm" font-weight="500">${lang}</text>
    ...
    const detailRow = (y, leftLbl, leftVal, rightLbl, rightVal) => `
        <g transform="translate(25, ${y})">
            <text x="0" y="0" class="sec text-sm">${leftLbl}</text>
            <text x="160" y="0" class="pri extrabold text-md text-anchor-end">${leftVal}</text>
            
            <text x="220" y="0" class="sec text-sm">${rightLbl}</text>
            <text x="380" y="0" class="pri extrabold text-md text-anchor-end">${rightVal}</text>
        </g>
    `;
    ...
    const detailHTML = `
        <g transform="translate(490, 480)">
            ...
            ${detailRow(85, "Total Repositories", user.public_repos, "Star's Count", stats.totalStars)}
            ${detailRow(125, "Fork's Count", stats.totalForks, "Commit's Count", stats.totalCommits)}
            ${detailRow(165, "Total PRs", stats.totalPRs, "PRs Merged", stats.totalPRsMerged)}
        </g>
    `;

    const bottomSection = langHTML + detailHTML;
